...
test.describe('포켓몬 타입별 필터 테스트', () => {
  ...
  for (const type of POKEMON_TYPES) {
    test(`'${type.kr}' 타입 필터가 올바르게 작동하는지 확인`, async ({ page }) => {
      // 해당 타입 링크 클릭
      ...
      await expect(page.locator(`text=${type.kr}`)).toBeVisible();
      
      // 포켓몬 목록이 표시되는지 확인
      await expect(page.locator('.grid')).toBeVisible();
      
      // 스크린샷 촬영 (실패 시 디버깅용)
      await page.screenshot({ path: `./screenshots/${type.en}-type-filter.png` });
    });
  }
});
...
test.describe('특정 포켓몬 검색 및 상세 정보 테스트', () => {
  ...
  for (const pokemon of POKEMON_TO_TEST) {
    test(`'${pokemon.name}' 검색 및 상세 정보가 올바르게 표시되는지 확인`, async ({ page }) => {
      // 검색창에 포켓몬 이름 입력
      ...
      // 스크린샷 촬영
      await page.screenshot({ path: `./screenshots/${pokemon.name}-detail.png` });
    });
  }
});
